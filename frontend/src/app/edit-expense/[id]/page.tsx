...
export default function EditExpensePage() {
  ...
  useEffect(() => {
    // Gider detayını çek
    fetch(`http://localhost:5264/api/expenses/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setAmount(data.amount);
        setCategoryId(data.categoryId);
        setDescription(data.description);
        setDate(data.date.substring(0, 10)); // YYYY-MM-DD formatı için
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`http://localhost:5264/api/expenses/${id}`, {
      method: "PUT",
